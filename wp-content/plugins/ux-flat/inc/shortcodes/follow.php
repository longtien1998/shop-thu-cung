 ...
 * @return string
 */
function uxf_flatsome_follow( $atts, $content = null, $tag = '' ) {
	...
	$wrapper_class = array( 'follow-icons' );

	if ( $class ) $wrapper_class[]      = $class;
	if ( $visibility ) $wrapper_class[] = $visibility;
	if ( !$invert ) $wrapper_class[] = 'social-icons';
	if ( $align ) {
	...
	$follow_links = apply_filters( 'flatsome_follow_links', array(
		...
		'zalo'     => array(
			...
			'atts'     => array(
				...
				'rel'        => 'nofollow',
				'class'      => $style . ' zalo',
				'title'      => esc_attr__( 'Zalo', 'flatsome' ),
				'aria-label' => esc_attr__( 'Zalo', 'flatsome' ),
			),
			'icon'     => get_flatsome_icon( 'icon-zalo' ),
			'priority' => 70,
		),
		'whatsapp'     => array(
			'enabled'  => ! empty( $whatsapp ),
			'atts'     => array(
				'href'       => $whatsapp,
				'data-label' => 'Whatsapp',
				'target'     => '_blank',
				...
				'title'      => esc_attr__( 'Whatsapp', 'flatsome' ),
				'aria-label' => esc_attr__( 'Whatsapp', 'flatsome' ),
			),
	...
	<div class="<?php echo esc_attr( implode( ' ', $wrapper_class ) ); ?>" <?php echo $scale; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
		<?php
		if ( $title ) echo '<span>' . esc_attr( $title ) . '</span>';
		foreach ( $follow_links as $follow_link ) {
			if ( isset( $follow_link['enabled'] ) && $follow_link['enabled'] == false ) continue;
			printf( '<a %1$s>%2$s</a>',
				flatsome_html_atts( $follow_link['atts'] ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
...
add_shortcode( 'follow', 'uxf_flatsome_follow' );
